...
class Trepn(Profiler):
    ...
    def load(self, device):
        ...
        time.sleep(3)  # launch_package returns instantly
        ...
        time.sleep(1)  # am force-stop returns instantly
    ...
    def aggregate_subject(self):
        ...
        for i in range(len(subject_rows[0])):
            l = [subject_rows[0][i]]
            util.write_to_file(filename, l, i)

    def aggregate_end(self, data_dir, output_file):
        rows = list()
        rows.append(self.aggregate_final(data_dir))
        for i in range(len(rows[0])):
            l = [rows[0][i]]
            util.write_to_file(output_file, l, i)
    ...
    def aggregate_trepn_subject(self, logs_dir):
        ...
        for i in range(len(runs)):
            current_dict = OrderedDict(
                sorted(list({k: v for k, v in list(runs[i].items())}.items()), key=lambda x: x[0]))
    ...
    # def aggregate_final1(self, data_dir):
    #     rows = []
        ...
        for subject in util.list_subdir(device_dir):
            ...
            for browser in util.list_subdir(subject_dir):
                ...
                if os.path.isdir(output_path):
                    for run_file in [f for f in os.listdir(output_path) if os.path.isfile(os.path.join(output_path, f))]:
                        if run_file != 'Aggregated.csv':
                            with open(os.path.join(output_path, run_file), 'r') as run:
                                ...
                                runs.append(run_dict)
                            for i in range(len(runs)):
                                list_dict = sorted(list({k: v for k, v in list(runs[i].items())}.items()), key=lambda x: x[0])
                                current_dict = OrderedDict()
